/* -*- mode: C -*- Time-stamp: "2010-12-22 22:57:24 jemarch"
 *
 *       File:         recfmt.c
  ...
  */
  ...
  while (*p
         ...
         && (matches.rm_so != -1))
    {
      ...
      /* Get the match.  */
      tmp_size = matches.rm_eo - matches.rm_so;
      tmp = xmalloc (tmp_size + 1);
      strncpy (tmp, p + matches.rm_so, tmp_size);
      tmp[tmp_size - 1] = '\0';

      /* Advance p. */
      p = p + matches.rm_eo;

      /* Parse a fex and validate it.  */
      fex = rec_fex_new (tmp + 2, REC_FEX_SUBSCRIPTS);
      if (!fex
          || (rec_fex_size (fex) != 1)
          || (rec_fex_elem_max (rec_fex_get (fex, 0)) != -1))
        {
          recutl_fatal (_("invalid slot '%s' in template.\n"), tmp + 2);
        }
      else
        {
          /* Get the corresponding field, if it exists.  */
          field_num = rec_fex_elem_min (rec_fex_get (fex, 0));
          if (field_num == -1)
            {
              field_num = 0;
            }
          ...
          if (field)
            {
              rec_buf_puts (rec_field_value (field), result_buf);
            }
        }
